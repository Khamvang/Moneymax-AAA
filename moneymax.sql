...
#Check and analize for make new approval RINGI
SELECT
	p.id 'contract_no',
	c.ncn ,
	cu.id 'customer_id',
	...
	cu.sec_contact_no 'customer_tel2',
	CONVERT (CAST(CONVERT(cu.occupation using latin1) as binary) using utf8) 'occupation',
	p.customer_monthly_income 'monthly_income',
	p.customer_monthly_expenditure 'monthly_expenditure',
	p.customer_monthly_profit 'monthly_profit',
	CASE WHEN CONVERT (CAST(CONVERT(cu.occupation using latin1) as binary) using utf8) like '%Ã Âºâ„¢Ã Â»â€°Ã ÂºÂ³%' THEN 'Ã Â»â‚¬Ã Âºâ€žÃ ÂºÂ·Ã Â»Ë†Ã ÂºÂ­Ã Âºâ€¡Ã Âºâ€�Ã ÂºÂ·Ã Â»Ë†Ã ÂºÂ¡'
		WHEN CONVERT (CAST(CONVERT(cu.occupation using latin1) as binary) using utf8) like '%Ã Âºâ„¢Ã Â»â€°Ã ÂºÂ³%Ã Âºï¿½Ã ÂºÂ´Ã Âºâ„¢%' THEN 'Ã Â»â‚¬Ã Âºâ€žÃ ÂºÂ·Ã Â»Ë†Ã ÂºÂ­Ã Âºâ€¡Ã Âºâ€�Ã ÂºÂ·Ã Â»Ë†Ã ÂºÂ¡ Ã Â»ï¿½Ã ÂºÂ¥Ã ÂºÂ° Ã ÂºÂ­Ã ÂºÂ²Ã ÂºÂ«Ã ÂºÂ²Ã Âºâ„¢'
		WHEN CONVERT (CAST(CONVERT(cu.occupation using latin1) as binary) using utf8) like '%Ã Â»â‚¬Ã Âºï¿½Ã ÂºÂµ%' THEN 'Ã Â»â‚¬Ã Âºâ€žÃ ÂºÂ·Ã Â»Ë†Ã ÂºÂ­Ã Âºâ€¡Ã Âºâ€�Ã ÂºÂ·Ã Â»Ë†Ã ÂºÂ¡ Ã Â»ï¿½Ã ÂºÂ¥Ã ÂºÂ° Ã ÂºÂ­Ã ÂºÂ²Ã ÂºÂ«Ã ÂºÂ²Ã Âºâ„¢'
		WHEN CONVERT (CAST(CONVERT(cu.occupation using latin1) as binary) using utf8) like '%Ã Â»â‚¬Ã Âºâ€šÃ ÂºÂ»Ã ÂºÂ²%Ã Âºâ€ºÃ ÂºÂ½Ã Âºï¿½%' THEN 'Ã Â»â‚¬Ã Âºâ€žÃ ÂºÂ·Ã Â»Ë†Ã ÂºÂ­Ã Âºâ€¡Ã Âºâ€�Ã ÂºÂ·Ã Â»Ë†Ã ÂºÂ¡ Ã Â»ï¿½Ã ÂºÂ¥Ã ÂºÂ° Ã ÂºÂ­Ã ÂºÂ²Ã ÂºÂ«Ã ÂºÂ²Ã Âºâ„¢'
		ELSE ''
	END 'category',
	CASE
		c.status WHEN 0 THEN 'Pending'
...
SELECT DATE_ADD(date(now()), INTERVAL - 10 DAY); -- return the date before now 10 days
SELECT DATE_ADD(date(now()), INTERVAL + 1 MONTH) ; --  return the date before now 2 months
...
where id in (11227,11220,11217,11009) 
and status = 1;
...
left join tblprospect p on (p.id = c.prospect_id)
left join tbldailyreport dr on (c.id = dr.contract_id)
where c.status in (4,6,7) and p.contract_type in (5,6,7,8) and dr.date in ('2020-11-30','2020-12-31'
